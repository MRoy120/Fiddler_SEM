...
#Decomp_2018_3 <- Decomp_2018_3 %>%
  #group_by(Replication, Density_NN) %>%
  #mutate(row_length = length(Replication)) %>%
  #ungroup()
...
Decomp_2018_Wider_2 <- filter(Decomp_2018_Wider_2, Replication %in% c("One",
                                                                      "Two",
                                                                      "Three",
                                                                      "Four"))

Decomp_2018_Wider_2 <- Decomp_2018_Wider_2 %>%
  dplyr::rename(depth_five = `5`) %>%
  dplyr::rename(depth_fifteen = `15`)
...
visreg(SB_Gamma_combined_glm, 
       ...
       strip.names = c("Final Soil Strength: 0", "Final Soil Strength: 10",
                       ...
                       "Final Soil Strength: 40", "Final Soil Strength: 50")
       ) +
  theme_bw() +
  #scale_y_continuous(limits = c(0, 35)) +
  My_Font_Sizes +
  theme(strip.text.x = element_text(size = 14, colour = "black", angle = 360)) +
  ggsave(filename = "2020_FSAD_SB_FSS_Visreg.png",
         width = 15, 
         height = 5)
...
#Wider
Burrows_combined_NBglm_wider <- glm.nb(data = Decomp_2018_Wider_2, 
                            Post_Burrow_Count ~ 
                              Density_Num *
                              Pre_Pene_AVG +
                              Site,
...
summary(Burrows_combined_NBglm_wider)
...
#Wider
Pre_Pene_combined_lm_wide <- lm(data = Decomp_2018_Wider_2, 
                           Pre_Pene_AVG ~
                             Site
                           )
summary(Pre_Pene_combined_lm_wide)
...
#Wider
Decomp_lm_wider <- lm(data = Decomp_2018_Wider_2,
                End_Mass_LB ~
                  Post_Burrow_Count *
                  depth_five +
                  depth_fifteen +
...
Burrows_Resid_PP <- combined_data_full %>%
  add_residuals(Burrows_combined_NBglm)
...
Burrows_Resid_PP <- Burrows_Resid_PP %>%
  add_predictions(Burrows_combined_NBglm)
